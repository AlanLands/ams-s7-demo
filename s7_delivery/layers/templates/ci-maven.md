...
id: ci-maven
layer: template
title: CI bootstrap — Maven (GitHub Actions)
stage: intake
summary: The GitHub Actions workflow S7 commits once to a connected Java/Maven repository as .github/workflows/s7-ci.yml, so a real developer push produces a real run that ci_sync reads back. Reports whether the build itself succeeded, so a run that never compiled cannot be read as a run that found no failures. Locked tokens are what ci_sync and the engine join on — the workflow name, the artifact name, the summary file and its keys, and the surefire/jacoco report paths.
locked: name: S7 CI, name: ci-summary, ci-summary.json, target/surefire-reports/TEST-*.xml, target/site/jacoco/jacoco.xml, test-exit-code, "build", "exit_code", "build_error", "tests_total", "tests_passed", "tests_failed", "coverage_pct", "tests": tests, "outcome"
---
name: S7 CI
on:
  push:
