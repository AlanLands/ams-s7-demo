...
import { ArrowLeft, Eye, Lock, Plus, RefreshCw, Save, Search, Undo2 } from 'lucide-react'
...
import type {
  FileSource, Impact, ProfileDetail, ProfileFileRow, ProfileGroup, ProfileLayer, ProfileSaveResult, WorkflowPreview,
} from '../types'
...
function NewFileForm({ name, onClose, onCreated }: { name: string; onClose: () => void; onCreated: (id: string) => void }) {
  const { run, busy } = useAdmin()
  const [layer, setLayer] = useState<ProfileLayer>('skill')
  ...
  const submit = async (e?: React.FormEvent) => {
    e?.preventDefault()
    setTouched(true)
    if (idErr || noteErr || bodyErr || undeclared.length || missing.length) return
    ...
    <Modal title={`New file in ${name}`} description="Adds a file to this profile only and records it as v1 in the profile's ledger. It does not exist in the default set, so nothing shows through under it." onClose={onClose} wide>
      <form onSubmit={submit}>
        <div className="form-grid">
          ...
          <Field label="Locked tokens" htmlFor="nf-locked" optional help="Comma-separated literal strings every later edit must keep — the lines a consumer depends on."
