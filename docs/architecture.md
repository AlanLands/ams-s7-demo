...
The Control Centre is the surface over this repo's delivery machinery. It
demonstrates the full governed
journey —
...
s7_delivery/factory/
  ...
  layers.py                   loader for the Rules and Skills layers below;
                              version ledger; the four-layer description
s7_delivery/layers/
  rules/<id>.md               the stable prompt prefix of one lane (verbatim)
  skills/<id>.md              the role text of one stage (verbatim)
  history.jsonl               append-only version ledger of both
```
...
## Relationship to the downstream lane

`s7_delivery/downstream.py` (the recorded three-role lane) is untouched;
`s7_delivery/factory/live.py` bridges into it for agentic stories in live and
replay runs. The factory shares its vocabulary — EPIC-S7-001, stories,
acceptance criteria, provenance labels — and keeps the same thin-HTTP pattern,
so the app stays a view over an engine that owns the rules. The Sprint-0
upstream pipeline (`pipeline.py`, `staged.py`) and the standalone intake app
were removed on 2026-09-03 once this engine had superseded both.
