...
| Run economics / KPIs | cross-cutting | **Both** | One ledger in `s7_delivery/factory/engine.py`, rendered twice |

This composes rather than duplicates: `s7_delivery/factory/` imports nothing
...
Summary (old ids alias). Docs: `docs/build-review-implementation-plan.md`,
`build-review-artifact-model.md`, `developer-workspace-model.md`,
`git-publication-model.md`, `build-review-state-machine.md`,
`build-review-demo-script.md`.
...
**Reporting written for a developer, added 2026-09-11.** The packs told a
coding agent what to *do* at each phrase and never what its report should
look like, so the start-phrase handoff on the first real story came back as
a governance document: rule numbers quoted at the reader, this file's own
process vocabulary verbatim, and six open questions written as open-ended
prose with no options and no recommendation. A developer read it and could
not tell what was being asked of them — which is the failure that matters,
because every phrase in this loop ends in a human decision, and a report
nobody can act on stalls a story exactly as surely as a missing test does.
So a **How to report back** section joins `git-workflow.md` (v6) and a
*Report for a human* rule joins `engineering-rules.md` (v5), with a
*Reporting* section in the pack's `AGENTS.md`: lead with state — which
story, which phase, what the developer's next word is — show the branch,
files, criteria and test results rather than narrating them, explain a
refusal by what it breaks and never by a rule number (the developer has not
read these files and should not need to), and keep the process words out of
it. Each open question becomes a decision answerable in a word: one line of
context, two or three **lettered options**, a marked recommendation with one
clause of why, and who owns the answer where it is the story author's rather
than the developer's — blocking questions first, quick confirmations after.
One discipline carries forward from the false-green fix of 2026-09-10 and is
now prohibited in as many words: **an unproven result is never reported as
verified** — a build that did not compile is not a red baseline, and a change
nobody ran is not working. Deterministic pack text as before (no model call,
no new provenance kind). The stale "plan it function by function" line in the
pack's `AGENTS.md`, left over from the per-function shape rejected on
2026-09-10, became "one acceptance criterion at a time" — which is what the
Build Discipline section below it already said. Existing runs pick the change
up on the next *Generate delivery packs* + publish, a human action as before.
...
4. **Must survive a port to a locked-down environment.** Plain Python +
   ...
   **Amended 2026-08-08 for the Control Centre frontend.** The Control Centre
   ...
   React + TypeScript + Vite (see `docs/design-history/plans/
   ...
   repo revisiting the wording until now. The build step is acceptable under
